...
class TrackingRepository:
    def __init__(self,engine,session_factory=None): self.engine,self.Session=resolve_database(engine,session_factory)
    def next_tracking_sequence(self):
        with self.Session() as s:return (s.scalar(select(func.count()).select_from(TrackingItemORM)) or 0)+1
    ...
    def scan_by_tracking_code(self,tracking_code):
        with self.Session() as s:
            ...
            return {"internal_id":item.internal_id,"product_id":item.product_id,"tracking_code":item.tracking_code,"product_code":product.product_code,"part_name":product.part_name,"customer":customer.name,"order_code":po.internal_order_code or po.po_number,"customer_po_number":po.po_number,"delivery_date":item.delivery_date.isoformat(),"quantity":str(item.quantity),"unit":item.unit,"status":item.status,"qr_status":item.qr_status}
    ...
    def add_operator(self,obj):
        with self.Session() as s:s.add(OperatorORM(**vals(obj)));s.commit()
        return obj
    def list_operators(self):
        with self.Session() as s:return tuple(s.scalars(select(OperatorORM).where(OperatorORM.active==True).order_by(OperatorORM.display_name)).all())
    ...
    def list_machining_types(self):
        with self.Session() as s:return tuple(s.scalars(select(MachiningTypeORM).where(MachiningTypeORM.active==True).order_by(MachiningTypeORM.display_order)).all())
    ...
    def set_preference(self,user_id,type_id):
        with self.Session() as s:s.merge(UserPreferenceORM(user_id=str(user_id),machining_type_id=str(type_id),updated_at=utc_now()));s.commit()
    ...
    def expand_attempt(self,item_id,type_id,new_max,user_id=None):
        ...
        with self.Session() as s:s.merge(AttemptDisplayORM(tracking_item_id=str(item_id),machining_type_id=str(type_id),max_visible_attempt=new_max,updated_at=utc_now(),updated_by=str(user_id) if user_id else None));s.commit()
        return new_max
    ...
    def history(self,item_id):
        with self.Session() as s:return tuple(self._report(x) for x in s.scalars(select(ProcessReportORM).where(ProcessReportORM.tracking_item_id==str(item_id)).order_by(ProcessReportORM.server_timestamp)).all())
