...
## R007B4 superseding decision

Open Design Cloud/AMR generation is retired from the Stage 5 hard gate. The
previous zero-artifact insufficient-balance run is preserved as historical
evidence and is not retried, recharged, or mutated. Open Design 0.19.0 may
remain installed as optional development tooling, but production runtime and
Stage 5 canonical design authority must not depend on Open Design.

The active design-toolchain decision is now Penpot MCP for canonical visual
design artifacts and UICanvas Local MCP for rapid local prototypes. Their live
integration gates are recorded in `PENPOT_INTEGRATION.md` and
`UICANVAS_INTEGRATION.md`; neither was connected during the R007B4 preflight.
